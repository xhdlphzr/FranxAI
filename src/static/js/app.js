...
eventSource.addEventListener("task_done", (e) => {
  ...
  const msgDiv = taskMessages[data.task_id];
  if (msgDiv) {
    msgDiv.innerHTML = `<div>${t("task.completed")}</div><div class="task-result">${escapeHtml(data.result || t("task.no_content"))}</div>`;
    delete taskMessages[data.task_id];
  }
});
eventSource.addEventListener("task_error", (e) => {
  ...
  const msgDiv = taskMessages[data.task_id];
  if (msgDiv) {
    msgDiv.innerHTML = `<div>${t("task.failed")}</div><div class="task-result">${t("chat.error", { message: escapeHtml(data.error) })}</div>`;
    delete taskMessages[data.task_id];
  }
});
eventSource.addEventListener("task_cancel", (e) => {
  ...
  const msgDiv = taskMessages[data.task_id];
  if (msgDiv) {
    msgDiv.innerHTML = `<div>${t("task.cancelled")}</div>`;
    delete taskMessages[data.task_id];
  }
});
async function stopTask(taskId) {
  try {
    const resp = await fetchWithAuth(`/cancel_task/${taskId}`, {
      method: "POST",
    });
    ...
    alert(t("task.stop_failed", { message: err.message }));
  }
}
            ...
            if (chunk.type === "text") {
              ...
              chunk.calls.forEach((tc) => {
                ...
                addToolCallBlockStructured(
                  ...
                  actualName,
                  actualParams,
                  resultContent || t("tool.no_result"),
                );
              });
            }
          });
...
(async function init() {
  ...
  if (tempToken) {
    ...
    localStorage.removeItem("FranxAgent_temp_token");
  }
