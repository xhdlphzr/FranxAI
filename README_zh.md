...
FranxAI 是一个轻量级的 AI 智能体框架，让 AI 能够读取文件、执行命令、搜索网络、理解多模态内容，并通过 MCP 协议真正操控世界。**现在，它还拥有「思绪回响」般的向量知识库：所有工具说明、技能文件、乃至对话历史，都会被自动向量化并持久存储。每次提问，AI 都能精准检索最相关的知识片段注入系统提示，实现长期记忆、跨会话回忆与动态知识增强。从此，AI 不仅能「从语言到世界」，更能「让每一次回响都成为思想的养分」。**
...
- 🧠 **智能记忆**：自动压缩历史对话，同时将完整对话历史存入 `knowledge/memories/` 目录，下次启动时自动加载，实现跨会话知识检索。
- ⏰ **定时任务**：后台线程运行，支持每天重复任务，让 AI 在指定时间自动执行指令。
- 📚 **技能系统**：将 `knowledge/` 文件夹下的所有 Markdown 文件自动合并为系统提示的一部分，赋予 AI 额外的知识、规则或工作流，扩展其能力边界。
