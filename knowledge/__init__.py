...
"""
Knowledge 模块：统一工具加载 + MCP 管理 + 自动构建向量知识库（支持增量更新）
"""
...
MEMORIES_DIR = KNOWLEDGE_ROOT / 'memories'   # 对话记忆备份目录（不参与自动扫描）

# 创建 memories 目录（如果不存在）
...
# ---------- 2. 收集所有 Markdown 文件（排除 memories 目录）----------
def _collect_md_files():
    """递归收集 knowledge/ 下所有 .md 文件，但排除 memories 目录"""
    files = []
...
def _get_file_state():
    """获取当前所有 .md 文件的状态（路径 -> (mtime, 内容)）"""
    state = {}
    for md_file in _collect_md_files():
        try:
            mtime = md_file.stat().st_mtime
            # 可选：同时存储内容哈希，但 mtime 足够用于检测修改
...
def _load_mcp_servers():
    """从配置文件加载 MCP 服务器并启动，将工具描述加入知识库（动态添加）"""
    global _mcp_tools
        ...
        try:
            ...
            with _mcp_lock:
                for tool in raw_tools:
                    ...
                    # 将描述动态插入向量库（如果尚未存在）
                    # 为避免重复，可以检查数据库中是否已有该描述，简单起见直接插入（可能重复，但去重由 search 的 rtn 处理）
...
# 启动 MCP 服务器（它们会调用 _add_document 动态添加描述）
_load_mcp_servers()
...
def _incremental_update():
    """
    增量更新：对比当前文件状态与 file_versions 表，对新增/修改的文件重新向量化并更新，
    对删除的文件从 vectors 表中删除对应的记录。
    """
    print("正在执行增量向量库更新...")
    ...
    for path, mtime in current_state.items():
        if path not in stored or stored[path] != mtime:
            ...
            try:
                ...
                print(f"⚠️ 更新文件 {path} 失败: {e}")

    # 2. 处理删除的文件（在 current_state 中不存在，但在 stored 中存在）
    for path in stored:
...
# 检查是否需要首次构建：若 vectors 表为空，则全量重建；否则增量更新
conn = sqlite3.connect(VECTOR_DB_PATH)
cursor = conn.cursor()
...
def search(query: str, k: int = 1, step: int = 1, rtn: list = None):
    """
    检索与 query 最相似的 k 条知识（自动去重，递归扩展）
    返回字符串列表（每条知识为纯文本）
    """
    if rtn is None:
        rtn = []
...
def add_conversation(user_msg: str, ai_msg: str):
    """
    将一轮问答动态插入向量库（实时生效），同时备份到 knowledge/memories/ 目录
    """
    text = f"用户：{user_msg}\nAI：{ai_msg}"
    # 插入向量库
