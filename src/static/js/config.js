...
function renderMCPServers() {
  ...
  if (mcpServers.length === 0) {
    container.innerHTML = `<div>${t("mcp.no_servers")}</div>`;
    return;
  }
  ...
  document.querySelectorAll(".mcp-server-item input").forEach((input) => {
    input.addEventListener("change", (e) => {
      ...
      if (field === "args") {
        ...
        mcpServers[idx][field] = e.target.value;
      }
    });
  });
...
function renderTasks() {
  ...
  if (entries.length === 0) {
    container.innerHTML = `<div>${t("task.no_tasks")}</div>`;
    return;
  }
  ...
  document.querySelectorAll(".delete-task").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      if (confirm(t("task.delete_confirm", { time }))) {
        await deleteTask(time);
        await loadTasks();
      }
    });
  });
...
async function addTask(time, content) {
  try {
    const resp = await fetchWithAuth("/tasks", {
      ...
      body: JSON.stringify({ action: "add", time, content }),
    });
    ...
    if (result.status === "success") return true;
      ...
      alert(
        t("task.add_failed", {
          message: result.error || t("task.unknown_error"),
        }),
      );
      return false;
    }
  } catch (e) {
    alert(t("chat.network_error", { message: e.message }));
    return false;
  }
}
...
async function loadConfig() {
  try {
    ...
    if (config.error)
      configStatus.innerText = t("config.load_failed", {
    ...
    configStatus.innerText = t("config.load_failed", { message: err.message });
  }
}
configForm.addEventListener("submit", async (e) => {
    ...
    tools: {
      ett: {
        ...
        temperature: parseFloat(
          document.getElementById("ett_temperature").value,
        ),
  ...
  try {
    ...
    const mergedConfig = {
      ...currentConfig,
      ...newConfig,
      tools: { ...currentConfig.tools, ...newConfig.tools },
    };
    const saveResp = await fetchWithAuth("/config", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    configStatus.innerText = t("config.save_failed", { message: err.message });
  }
});
